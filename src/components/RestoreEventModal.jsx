...
export default function RestoreEventModal({ restoreLaneId, setRestoreLaneId }) {
  const { events, files, currentFileId, handleRestoreEvent } = useAppStore();
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#fff', padding: '20px', borderRadius: '8px', width: '400px', maxHeight: '80vh', overflowY: 'auto' }}>
        ...
        <p style={{ fontSize: '12px', color: '#666' }}>年表「{currentTl.title}」の除外イベント</p>
          ...
          {currentTl.excludedEventIds.map(eventId => {
            ...
            return (
              <li key={eventId} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid #eee' }}>
                ...
                <button onClick={() => { handleRestoreEvent(eventId, restoreLaneId); if (currentTl.excludedEventIds.length <= 1) setRestoreLaneId(null); }} style={{ padding: '4px 12px', background: '#000', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>復帰</button>
              </li>
            );
          })}
        </ul>
